...
- ✅ **Listagem de Filmes por Seção** — Populares, Melhores Avaliados e seções por categoria (Ação, Comédia), cada uma num carrossel horizontal com setas que desabilitam nas pontas (início/fim da lista) e a posição de scroll lembrada ao voltar pra tela.
...
- ✅ **Navegação** — Angular Router entre listagem, busca e detalhes. Um botão "voltar" existe só na tela de detalhes e usa o histórico real do navegador, restaurando a página de origem (home ou busca) na posição exata de scroll — página e carrossel — em que estava; já o logo "Catálogo" no topo é a saída "começar do zero": sempre leva pra home e zera essa memória de scroll.
...
- **Busca combinada (título + ator + diretor) num campo só.** A TMDB não tem um endpoint que combine busca textual com filtro por pessoa. `Tmdb.search()` dispara duas chamadas em paralelo — `/search/movie` para título, e `/search/person` → `/discover/movie?with_people=` para ator/diretor (a mesma pessoa pode aparecer como elenco ou equipe técnica, então um único `with_people` cobre os dois) — e mescla os resultados removendo duplicatas por id. Trocamos uma UI "mais correta" (com modos de busca separados: título/ator/diretor/categoria) por uma experiência mais simples de usar — um campo só, resultado aproximado, sem problema.
  ...
  **Limitação conhecida:** como cada página mescla a página N de `/search/movie` com a página N de `/discover/movie` e só depois remove duplicatas, o total de itens únicos numa página pode ficar abaixo de 20 sempre que as duas fontes se sobrepõem ou uma delas devolve pouca coisa naquela página — sem nenhum reaproveitamento da página seguinte pra completar. Na prática, a tela de busca às vezes mostra uma página parcial (com espaço sobrando na grade) mesmo tendo mais resultados na próxima página. Corrigir isso exigiria transformar `search()` num loop que continua puxando páginas extras de qualquer uma das fontes até fechar 20 itens únicos (ou esgotar as duas), o que tira o mapeamento direto de "página N" pra página N das APIs de origem — passaria a precisar rastrear quanto de cada fonte já foi consumido pra saber de onde continuar na página seguinte, em vez da chamada stateless atual.
...
- **Barra de busca do topo: limpa só ao voltar pra home, preservada em qualquer outra tela.** A barra fixa no topo (`app.html`) não lê `ActivatedRoute.queryParamMap` (isso zeraria o campo em qualquer rota sem `title`, inclusive nos detalhes do filme). Em vez disso, o componente raiz (`App`) escuta `Router.events` filtrando por `NavigationEnd` e decide o valor pelo path exato: em `/` o campo é limpo; em `/search` é preenchido com o `title` da URL; em qualquer outra rota (como `/movie/:id`) o valor atual é mantido intocado. Resultado: abrir um filme a partir de um resultado de busca não apaga o que foi buscado, e tanto voltar pra home pelo botão "voltar" dos detalhes quanto pelo logo "Catálogo" limpam o campo (a diferença entre os dois fica na memória de scroll dos carrosséis — ver abaixo).

- **Dois jeitos de "voltar" pra home, com intenções diferentes.** O botão "voltar" existe só na tela de detalhes (`movie-details`) e chama `Location.back()` — navegação real pelo histórico do navegador, então volta pra exata página de origem (home ou busca, com os mesmos query params) e aciona a restauração de scroll já existente (página e `CarouselScrollMemory`), como se o usuário nunca tivesse saído. Já o logo "Catálogo" no topo (`app.html`) é sempre uma navegação nova pra `/`, não um "voltar" — e por isso o componente raiz chama `CarouselScrollMemory.clear()` no clique, antes mesmo do Router navegar: é a saída explícita de "recomeçar do zero", então os carrosséis da home voltam todos centralizados em vez de restaurar a última posição rolada.
...
- **Tema escuro via `theme-type: dark` do Material, não recolorindo cada componente na mão.** Trocar `theme-type` no `mat.theme()` já ajustou automaticamente botões, ícones, spinner e bordas de input pros tokens corretos — evitando sobrescrever cor por cor manualmente onde o Material já resolve isso sozinho. Por cima disso, o fundo da página é fixado no hex exato do design (`#2e3349`) via CSS puro, já que o tom escuro derivado automaticamente pelo Material não bateria com esse valor específico. O feedback de hover (borda branca) usa `ring` do Tailwind (baseado em `box-shadow`, não `border`) para não deslocar layout nenhum — com o cuidado de aplicar o anel sempre no elemento mais externo não afetado por `overflow-hidden`/`overflow-x-auto` de algum ancestral, já que ambos cortam `box-shadow` que "vaza" pra fora da própria caixa. Mesmo com esse cuidado, a folga ao redor dos cards do carrossel (`py-1` na vertical, e nenhum padding na horizontal) era curta demais pra um `ring-2` (2px): o anel de hover chegava a ser cortado no topo, e no primeiro/último card da lista (sem nada "espiando" do lado de fora pra dar folga) também cortava na lateral. Corrigido dando padding nos dois eixos do container de scroll (`px-3 py-3`, 12px), já que o scroll horizontal por si só limita onde o anel pode "vazar".

- **Container principal mais largo (`max-w-7xl`).** O `<main>` em `app.html` — compartilhado por todas as páginas — passou de `max-w-5xl` pra `max-w-7xl`, dando mais espaço horizontal em telas grandes (mais um card visível por carrossel) e, como efeito colateral, menos pressão nas bordas onde o anel de hover podia ficar espremido.
